...
if (isset($_GET['id'])) {
  ...
  $dynamicList = "";
  $sql = $connection->query("SELECT * FROM slusalice WHERE id='$id' LIMIT 1");
  $productCount = mysqli_num_rows($sql); // count the output amount
    ...
    while($row = mysqli_fetch_array($sql)){ 
       ...
       $Frekventni_odziv = $row["Frekventni_odziv"];
       $Boja = $row ["Boja"];
       $slika=$row["slika"];
       $id=$row["id"];
...
<div class="col-12 col-md-12">
        <h6>Specifikacije slušalica:</h6><br>
  ...
  <tbody>
     ...
     <tr>
      ...
      <td>Mikrofon:</td>
      <td><?php echo $Mikrofon; ?></td>
  
    </tr>
     <tr>
      
      <td>Frekventni odziv:</td>
      <td><?php echo $Frekventni_odziv; ?></td>
