...
if (isset($_POST['btnn-login'])) {
    ...
    $query = $connection->query("SELECT id, email, password FROM korisnici WHERE email='$email'");
    $row=$query->fetch_array();
    
    $count = $query->num_rows; // if email/password are correct returns must be 1 row
    
    if (password_verify($password, $row['password']) && $count==1) {
        $_SESSION['userSessionn'] = $row['id'];
        header("Location: Unos.php");
    } else {
        $msg = "<div class='alert alert-danger'>
                    <span class='glyphicon glyphicon-info-sign'></span> &nbsp; Pogrešan email ili password! !
                </div>";
    }
    $connection->close();
}
...
<div class="signin-form">
    ...
    <div class="container">
       ...
       <form class="form-signin" method="post" id="login-form">
        ...
        <h2 class="form-signin-heading">Prijava za placanje</h2><hr />
        ...
        <div class="form-group">
            <button type="submit" class="btn btn-primary" name="btnn-login" id="btnn-login">
            <span class="glyphicon glyphicon-log-in"></span> &nbsp; Prijava
            </button> 
            
            <a href="Novi_nalog.php" class="btn btn-primary" style="float:right;">Napravite nalog</a>
